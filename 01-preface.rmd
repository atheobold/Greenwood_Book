...
```{r}
terms_chp_1 <- c(terms_chp_1, "hypothesis testing", "confidence interval")
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "quantitative", "categorical", "response", 
                 "explanatory")
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "linear model", "Chi-Square Test", 
                 "scope of inference")
```
...
knitr::include_graphics('01/images/methods_edit.PNG')
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "assumptions", "interaction", 
...
```{r}
terms_chp_1 <- c(terms_chp_1, "Chi-Square Homogeneity Test", 
...
```{r}
terms_chp_1 <- c(terms_chp_1, "transformation", "multiple linear regression")
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "random sampling", "random assignment", 
                 "causal inference")
```
...
knitr::include_graphics('01/images/fig1-2_alt.jpg')
```
...
knitr::include_graphics('01/images/data-frame.svg')
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "reproducible", "replication")
```
...
knitr::include_graphics('01/images/alt_fig 1-3.png')
```
...
in the study was `r mean(treadmill$RunTime)` minutes with a standard
deviation of `r sd(treadmill$RunTime)` minutes. But you should remember that
...
```{r}
terms_chp_1 <- c(terms_chp_1, "symmetric distribution")
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "five number summary")
```
...
```{r Figure1-5, echo = FALSE, fig.cap = "(ref:fig1-5)", out.width = "100%", fig.align = "center"}
knitr::include_graphics('01/images/quantile_edits.jpg')
```
...
knitr::include_graphics('01/images/pipe-operator.png')
```
...
```{r,out.width = "50%", fig.align = "center"}
knitr::include_graphics('01/images/pipe-operator2.png')
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "skewed distribution", 
                 "outlier", "histogram")
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "boxplot")
```
...
```{r}
terms_chp_1 <- c(terms_chp_1, "IQR (inter-quartile range)")
```
...
before, Q1 was `r quantile(treadmill$RunTime, probs = 0.25)` and Q3 was 
`r quantile(treadmill$RunTime, probs = 0.75)`, providing an IQR of:
...
Based on this result the maximum (`r max(treadmill$RunTime)`) was flagged as a
potential outlier, because it was larger than than Q3 $+1.5*$IQR, which was 
`r quantile(treadmill$RunTime, probs = 0.75) + 1.5*IQR(treadmill$RunTime)`. 
...
```{r,echo = F,fig.width = 5, fig.height = 3.5, echo = TRUE}
ggplot(data = treadmill, aes(y = RunTime)) + 
  geom_boxplot() + 
...
* `r colFmt("OBEJECT_NAME","red")` ``<-`` **read_csv(**`r colFmt('"path_to_csv_file/FILE_NAME.csv"','red')`**)**
...
* `r colFmt("DATASET_NAME","red")`**$**`r colFmt("VARIABLE_NAME","red")`

    * To access a particular variable in a tibble called 
    `r colFmt("DATASET_NAME","red")`, use a $ and then the
    `r colFmt("VARIABLE_NAME","red")`. 

* **glimpse(**`r colFmt("DATASET_NAME","red")`**)**
...
* **mean(**`r colFmt("VARIABLE_NAME","red")`**)**
...
* **min(**`r colFmt("VARIABLE_NAME","red")`**)**
...
* **max(**`r colFmt("VARIABLE_NAME","red")`**)**

    * Calculates the maximum of the observations in a variable, where the 
    variable is required to be numerical. 
    \index{\texttt{max()}|textbf}
    
* **quantile(**`r colFmt("VARIABLE_NAME","red")`, 
              **probs =** `r colFmt("PERCENT_OF_DATA_BELOW","red")`**)**
...
* **summarize(**`r colFmt("DATASET_NAME","red")`, 
              `r colFmt("DATA_SUMMARIES","red")`**)** 
...
* **ggplot(data =** `r colFmt("DATASET_NAME","red")`, 
           **aes(x =** `r colFmt("VARIABLE","red")`, 
               **y =** `r colFmt("VARIABLE_NAME","red")`**))** 
...
* **geom_density(fill =** "`r colFmt("COLOR_OF_CHOICE","red")`"**)**
